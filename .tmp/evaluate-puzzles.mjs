...
var outputFile = `${outputDir}/scores.json`;
var RULE_WEIGHT_DEFAULT = 10;
var RULE_WEIGHTS = [
  ["\u5DF2\u63ED\u5F00\u7684\u725B", 1],
  ["\u552F\u4E00\u989C\u8272", 2],
  ["\u552F\u4E00\u6D3B\u8DC3\u884C", 3],
...
function applyCowReveal(sim, truth, row, col, state) {
...
function solveAndScore(puzzle, maxStepsFactor = 4) {
  ...
  for (let step = 0; step < maxSteps; step++) {
    if (state.cowsFound >= puzzle.n) {
      steps = step;
      const totalScore2 = Object.entries(usage).reduce((sum, [ruleName, cnt]) => {
        return sum + ruleWeight(ruleName) * cnt;
      }, 0);
      return { solved: true, steps, ruleUsage: usage, totalScore: totalScore2 };
    }
...
async function main() {
  ...
  if (fileArg) {
    ...
    const modes = modeArg ? [modeArg] : Object.keys(manifest);
  ...
  const scored = Object.entries(results).map(([file, r]) => ({ file, ...r })).sort((a, b) => b.totalScore - a.totalScore);
  const output = {
