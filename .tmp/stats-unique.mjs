...
function passesUnique(game, requireUnique) {
  return !requireUnique || countCowSolutions(game, 2) === 1;
}
function acceptGame(game, hintCheck, requireUnique) {
...
function createGameState(n2, mode = "hard", options) {
  ...
  const builders = layout === "grow" ? [tryGrow, trySnake] : layout === "snake" ? [trySnake, tryGrow] : requireUnique || hintCheck ? [tryGrow, trySnake] : [trySnake, tryGrow];
