...
function tryInferFullCowPositions(n, colorGrid) {
...
function inferCowsByHintDeduction(n, colorGrid) {
  const testBoard = buildBoardFromColorGrid(n, colorGrid);
  const seenHints = /* @__PURE__ */ new Set();
  ...
  function revealDeducedCow(row, col) {
    const cell = testBoard.grid[row][col];
    if (cell.isRevealed) return;
    cell.hasCow = true;
    cell.isRevealed = true;
...
function inferCowsForImport(n, colorGrid) {
  const full = tryInferFullCowPositions(n, colorGrid);
  if (full && full.length === n) {
    return { cows: full, note: "\u5DF2\u6839\u636E\u989C\u8272\u5E03\u5C40\u63A8\u65AD\u5168\u90E8\u725B\u7684\u4F4D\u7F6E" };
  }
  const deduced = inferCowsByHintDeduction(n, colorGrid);
  if (deduced.length > 0) {
    const note = deduced.length === n ? "\u5DF2\u901A\u8FC7\u63A8\u6F14\u94FE\u63A8\u65AD\u5168\u90E8\u725B\u7684\u4F4D\u7F6E" : `\u5DF2\u901A\u8FC7\u63A8\u6F14\u94FE\u63A8\u65AD ${deduced.length}/${n} \u5934\u725B\uFF08\u5E03\u5C40\u53EF\u80FD\u65E0\u89E3\u6216\u4E0D\u5B8C\u6574\uFF09`;
    return { cows: deduced, note };
  }
  ...
  return { cows: [], note: "\u672A\u80FD\u63A8\u65AD\u725B\u7684\u4F4D\u7F6E\uFF0C\u5DF2\u6309\u65E0\u725B\u5E03\u5C40\u5BFC\u5165" };
}
...
console.log(JSON.stringify(parsePuzzleImport(JSON.stringify(sample)), null, 2));
