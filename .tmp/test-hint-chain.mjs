...
// test-hint-chain.ts
var sizes = [4, 5, 6, 8, 10, 12, 15];
var trials = 3;
for (const n of sizes) {
  console.log(`checking ${n}x${n}...`);
  for (let trial = 0; trial < trials; trial++) {
    const game = createGameState(n, "easy");
    if (!isHintSolvable(game)) {
      throw new Error(`Generated ${n}x${n} puzzle is not solvable by hint chain on trial ${trial}`);
    }
  }
}
console.log(`PASS hint-chain generation check (${sizes.length * trials} puzzles)`);
