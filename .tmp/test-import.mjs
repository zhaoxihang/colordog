...
function buildBoardFromColorGrid(n, colorGrid) {
  return {
    n,
    grid: colorGrid.map(
...
function solutionSetToCows(solution) {
  return [...solution].map((key) => {
    const [r, c] = key.split(",").map(Number);
    return [r, c];
  });
}
function inferCowPositions(n, colorGrid) {
  const board = buildBoardFromColorGrid(n, colorGrid);
  const maxNodes = Math.min(12e4, 8e3 + n * n * 600);
  ...
  if (truncated) {
    return { error: "\u653E\u725B\u65B9\u6848\u8FC7\u591A\uFF0C\u65E0\u6CD5\u81EA\u52A8\u63A8\u65AD\uFF08\u8BF7\u624B\u52A8\u63D0\u4F9B cows\uFF09" };
  }
  if (solutions.length === 0) {
    return { error: "\u8BE5\u989C\u8272\u5E03\u5C40\u65E0\u89E3\uFF08\u65E0\u6CD5\u6EE1\u8DB3\u884C/\u5217/\u989C\u8272/\u4E0D\u76F8\u90BB\u7EA6\u675F\uFF09" };
  }
  if (solutions.length === 1) {
    return { cows: solutionSetToCows(solutions[0]) };
  }
  ...
  if (hintSolvable.length === 1) {
    return { cows: solutionSetToCows(hintSolvable[0]) };
  }
  if (hintSolvable.length > 1) {
    return { error: `\u5B58\u5728 ${solutions.length} \u79CD\u653E\u725B\u65B9\u6848\u4E14\u591A\u79CD\u5747\u53EF\u63D0\u793A\u94FE\u901A\u5173\uFF0C\u8BF7\u624B\u52A8\u63D0\u4F9B cows` };
  }
  return { error: `\u5B58\u5728 ${solutions.length} \u79CD\u653E\u725B\u65B9\u6848\uFF0C\u65E0\u6CD5\u7528\u63D0\u793A\u94FE\u552F\u4E00\u63A8\u65AD\uFF0C\u8BF7\u624B\u52A8\u63D0\u4F9B cows` };
}

// src/utils/puzzleImport.ts
function isGameMode(value) {
  return value === "easy" || value === "hard";
}
function validateColorGrid(n, colorGrid) {
...
function parsePuzzleImport(json) {
  let raw;
  try {
    raw = JSON.parse(json);
  } catch {
  ...
  if (!validateColorGrid(n, data.colorGrid)) {
    return { ok: false, reason: `colorGrid \u987B\u4E3A ${n}\xD7${n} \u7684\u975E\u8D1F\u6574\u6570\u989C\u8272\u77E9\u9635` };
  }
  const colorGrid = data.colorGrid;
  let cows;
  let cowsInferred = false;
  if (data.cows === void 0 || data.cows === null) {
    const inferred = inferCowPositions(n, colorGrid);
    if ("error" in inferred) {
      return { ok: false, reason: inferred.error };
    }
    cows = inferred.cows;
    cowsInferred = true;
  } else if (!validateCows(n, data.cows)) {
    return { ok: false, reason: `cows \u987B\u4E3A ${n} \u4E2A\u4E0D\u91CD\u590D\u7684 [\u884C, \u5217] \u5750\u6807` };
  } else {
    cows = data.cows;
  }
  if (cows.length !== n) {
    return { ok: false, reason: `\u9700\u8981\u6070\u597D ${n} \u5934\u725B` };
  }
  return {
    ok: true,
